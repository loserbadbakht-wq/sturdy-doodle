 ...
 * GET  /?url=<txt>    -> scans the txt link and lists configs that USE UDP (default)
 * GET  /?url=<txt>&mode=all -> lists every parsed config
 * GET  /api?url=<txt> -> same, but returns JSON
 *
 * A config is classified as UDP ONLY when the link itself declares UDP:
 *   • udp=1 / udp=true / udp-relay=1 / udp_relay=1 ...
 *   • network=udp
 *   • plugin=v2ray-plugin or plugin=xray-plugin (UDP is tunnelled)
 * Everything else — including plain ss:// with no UDP flag — is NOT UDP.
 */
...
// Cap rendered rows so a huge "All" view does not lock up the browser.
const MAX_ROWS = 500;
...
/* ------------------------------------------------------------------ */
/* UDP detection — strictly "the link declares UDP"                    */
/* ------------------------------------------------------------------ */
...
function detectUdp(cfg) {
  ...
  for (const key of ['udp', 'udp-relay', 'udp_relay', 'udprelay']) {
    if (p[key] !== undefined && p[key] !== '') {
      ...
      if (on) return { udp: true, reason: `link declares ${key}=${p[key]}` };
      return { udp: false, reason: `link declares ${key}=${p[key]} (disabled)` };
    }
  }

  if (p.network && /(^|[,\s])udp([,\s]|$)/i.test(String(p.network))) {
    return { udp: true, reason: `link declares network=${p.network}` };
  }
  ...
  if (plugin) {
    ...
    return { udp: false, reason: `plugin "${plugin}" — no UDP declared` };
  }

  if (cfg.type === 'ssr') {
    return { udp: false, reason: 'ShadowsocksR has no UDP relay (TCP only)' };
  }

  return { udp: false, reason: 'no UDP flag in link — not confirmed as UDP' };
}
...
async function scan(target) {
  ...
  const stats = {
    ...
    notUdp: configs.filter((c) => !c.udp).length,
...
const STYLE = `
  ...
  *{box-sizing:border-box}
  ...
  .tab{
    ...
    padding:8px 16px;font-size:13px;color:var(--muted);text-decoration:none;font-weight:600;
    display:inline-flex;align-items:center;gap:6px;
  }
  .tab:hover{background:#1c2128;color:var(--fg)}
  .tab.active{background:var(--accent);border-color:var(--accent);color:#03121f}
  .tab.active .count{color:#03121f}
  .tab .count{
    background:#ffffff1a;border-radius:999px;padding:0 8px;font-size:12px;color:var(--muted);
    font-weight:700;line-height:18px;
  }
  .tab.active .count{background:#0000001a}
  ...
  @media (max-width: 760px){
    ...
    input[type=url]{flex:1 1 100%;font-size:16px}
...
function renderRows(list) {
  ...
  list.forEach((c, i) => {
    ...
    html +=
      ...
      `<td data-label="UDP">${c.udp ? '<span class="yes">✔ UDP</span>' : '<span class="no">✖ no</span>'}</td>` +
...
function renderPage({ target, mode, result, error }) {
  let body = '';
  ...
  if (result) {
    ...
    const udpList = result.configs.filter((c) => c.udp);
    const fullList = mode === 'all' ? result.configs : udpList;

    let displayList = fullList;
    let truncated = false;
    if (fullList.length > MAX_ROWS) {
      displayList = fullList.slice(0, MAX_ROWS);
      truncated = true;
    }

    // ---- tabs (default = udp) ----
    const q = encodeURIComponent(target);
    const udpHref = `?url=${q}&mode=udp`;
    const allHref = `?url=${q}&mode=all`;
    body += `<div class="tabs">
      <a class="tab ${mode !== 'all' ? 'active' : ''}" href="${udpHref}">
        UDP only <span class="count">${s.udp}</span>
      </a>
      <a class="tab ${mode === 'all' ? 'active' : ''}" href="${allHref}">
        All configs <span class="count">${s.total}</span>
      </a>
    </div>`;
    ...
    body += `<div class="stats">
      ...
      <span class="pill no">Not UDP: <b>${s.notUdp}</b></span>
    ...
    if (truncated) {
      body += `<div class="alert warn">Showing the first <b>${MAX_ROWS}</b> of
        <b>${fullList.length}</b> rows for performance. Use the
        <a href="/api?url=${q}" style="color:var(--accent)">JSON API</a> to get the full list.</div>`;
    }

    if (s.total > 0 && !displayList.length) {
      if (mode === 'all') {
        body += `<div class="alert">No configs parsed at all. Try the URL again or check the file.</div>`;
      } else {
        body += `<div class="alert">No configs declared UDP — none of the parsed links enable UDP.
          Switch to the <b>All configs</b> tab to see everything that was parsed.</div>`;
      }
    } else if (displayList.length) {
      body += renderRows(displayList);
    }

    body += `<p class="hint">JSON API:
      <a href="/api?url=${q}">/api?url=…</a>
      &nbsp;·&nbsp; current view:
      <b>${mode === 'all' ? 'all configs' : 'UDP only'}</b></p>`;
  }
...
<div class="wrap">
  ...
  <p class="sub">Paste a subscription <code>.txt</code> link — the worker fetches it, parses every <code>ss://</code> and <code>ssr://</code> config and lists the ones whose link <b>declares UDP</b>.</p>
  ...
  <form method="GET" action="/">
    ...
    <input type="hidden" name="mode" value="${mode === 'all' ? 'all' : 'udp'}">
  ...
  <footer>
    <b>A config is counted as UDP only when the link itself declares it</b><br>
    • <code>udp=1</code> / <code>udp=true</code> / <code>udp-relay=1</code> / <code>udp_relay=1</code> …<br>
    • <code>network=udp</code><br>
    • <code>plugin=v2ray-plugin</code> or <code>plugin=xray-plugin</code> (UDP is tunnelled)<br>
    <br>
    <b>Everything else is treated as NOT UDP</b><br>
    • <code>ssr://</code> — ShadowsocksR has no UDP relay (TCP only).<br>
    • <code>ss://</code> with an <code>obfs-local</code> / <code>simple-obfs</code> plugin — TCP only.<br>
    • <code>ss://</code> with no UDP flag — plain Shadowsocks <i>can</i> relay UDP, but the link
      doesn't say so, so it is not counted.<br>
    <br>
    A Worker cannot send real UDP packets, so this is a link-declaration check, not a live probe.
  </footer>
</div>
...
export default {
  async fetch(request) {
    ...
    // default = udp; "all" (or legacy "?all=1") switches to showing every config
    const mode =
      modeParam === 'all' || url.searchParams.has('all') ? 'all' : 'udp';
    ...
    return new Response(renderPage({ target, mode, result, error }), {
      headers: {
