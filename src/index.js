...
function detectUdp(cfg) {
  ...
  // 1) explicit declaration inside the link
...
const STYLE = `
  ...
  *{box-sizing:border-box}
  ...
  input[type=url]{
    flex:1 1 420px;min-width:220px;background:#0d1117;border:1px solid var(--border);border-radius:8px;
    color:var(--fg);padding:11px 13px;font-size:14px;outline:none;
  }
  ...
  .pill{background:var(--panel);border:1px solid var(--border);border-radius:999px;padding:5px 12px;font-size:13px;color:var(--muted)}
  .pill b{color:var(--fg)}
  ...
  .why{color:var(--muted);max-width:280px}
...
function renderRows(list) {
  ...
  list.forEach((c, i) => {
    ...
    html +=
      ...
      `<td class="mono">${esc(c.port)}</td>` +
      `<td class="mono">${esc(c.method)}</td>` +
      `<td class="mono">${esc(middle)}</td>` +
      `<td>${c.udp ? '<span class="yes">✔ UDP</span>' : '<span class="no">✖ TCP</span>'}</td>` +
      `<td class="why">${esc(c.udpReason)}</td>` +
      `<td>${esc(c.remarks)}</td>` +
      `<td class="raw"><button type="button" data-raw="${esc(c.raw)}" onclick="copyRaw(this)">copy</button></td>` +
      '</tr>';
  });
...
<div class="wrap">
  ...
  <form method="GET" action="/">
    <input type="url" name="url" placeholder="https://example.com/sub.txt" value="${esc(target)}" required>
