 ...
 *  Every generated config routes through the reverse proxy path.
 * ============================================================ */
  ...
  async fetch(request, env, ctx) {
    try {
      ...
      if (upgrade === "websocket") {
        // V-Bridge path: /<host>:<port>/<anything>  →  set fallback proxy
...
/* ============================================================
 *  4. Config builders  (ALL routes go through V-Bridge path)
 * ============================================================ */
...
/** V-Bridge path segment for a node: "<ip>:<port>" (brackets stripped) */
const bridgeTarget = (n) => `${n.ip.replace(/[\[\]]/g, '')}:${n.pt}`;

/**
 * Build a vless:// link that:
 *   - connects to the Worker domain on port 443 (TLS always)
 *   - uses path "/<realIP>:<realPort>/?ed=2560"  → picked up by VLESS handler
 */
function nodeLink(userID, hostName, n) {
  const target = bridgeTarget(n);
  const path = encodeURIComponent(`/${target}/`).replace(/%2F/g, '%2F') + '%3Fed%3D2560';
