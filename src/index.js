 ...
 * GET  /?url=<txt>    -> scans the txt link and lists configs that USE UDP
...
function detectUdp(cfg) {
  ...
  // 1) explicit UDP flag in the link
  ...
  // 4) SSR never relays UDP
  if (cfg.type === 'ssr') {
    return { udp: false, reason: 'ShadowsocksR has no UDP relay (TCP only)' };
  }

  // 5) plain ss:// — nothing declared, so it does NOT count as UDP
...
function renderPage({ target, showAll, result, error }) {
  ...
  if (result) {
    ...
    body += `<p class="hint">${
      showAll
        ? 'Showing <b>all</b> parsed configs.'
        : 'Showing only configs whose link <b>declares UDP</b>.'
    ...
    if (s.total > 0 && !list.length) {
      body += `<div class="alert">No configs declared UDP — none of the parsed links enable UDP.
        Tick <b>show all configs</b> to see everything that was parsed.</div>`;
    } else if (list.length) {
